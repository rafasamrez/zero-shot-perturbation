...
def preprocess_expression(X: np.ndarray) -> np.ndarray:
    """Normalise raw FPKM to log1p(CPM × 1e4).

    Matches the ``_normalize_and_log`` function in ``utils.py`` exactly:
    scales each sample to ``_TARGET_SUM`` total counts, then applies log1p.

    Parameters
    ----------
    X : np.ndarray
        Raw FPKM expression matrix, shape ``(n_samples, n_genes)``.

    Returns
    -------
    np.ndarray
        Normalised matrix, dtype float64, same shape as input.
    """
    X = X.astype(np.float64)
    row_sums = np.nansum(X, axis=1, keepdims=True)
    row_sums[row_sums == 0] = 1.0
    X = X * _TARGET_SUM / row_sums
    return np.log1p(X)
...
def run_perturbation_pipeline(
    ...
    for disease_abbrev, disease_group in benchmark.groupby("disease_abbrev"):
        ...
        adata = load_cohort_data(disease_abbrev)
        ...
        for _, row in disease_group.iterrows():
            ...
            target_genes_raw = row["target_genes"]   # comma-separated Entrez IDs
            expected         = row["expected_efficacy"]

            target_gene_ids = [
                int(g.strip()) for g in str(target_genes_raw).split(",")
            ]
